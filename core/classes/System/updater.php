...
class updater 
{
     ...
     * и тд
     */        
    public $UPDATE_CHECK_DAY_OF_WEEK_INDEX;

    // папка куда сохраняем бэкап
    public $GET_PATH_TO_BACKUP_FOLDER;

    // папка куда сохраняется скачанный архив с обновлением 
    public $GET_PATH_TO_TEMP_UPDATE_FOLDER;

    // путь к файлу с текущей версией 
    public $GET_PATH_TO_LOCAL_VERSION_FILE;

    // ссылка на версию на сервере
    public $GET_PATH_TO_SERVER_VERSION_FILE;

    // архив с обновлением
    public $GET_PATH_TO_DOWNLOADED_UPDATE_FILE;


    public function __construct()
    { 
        $this->UPDATE_CHECK_DAY_OF_WEEK_INDEX =  5;

        $this->GET_PATH_TO_BACKUP_FOLDER =  $_SERVER['DOCUMENT_ROOT'].'/backup/';
        
        $this->GET_PATH_TO_TEMP_UPDATE_FOLDER = $_SERVER['DOCUMENT_ROOT'].'/update/';
        
        $this->GET_PATH_TO_LOCAL_VERSION_FILE = $_SERVER['DOCUMENT_ROOT'].'/version.txt';

        $this->GET_PATH_TO_SERVER_VERSION_FILE = 'https://raw.githubusercontent.com/lime-sklad/update_limestore/master/version.txt';

        $this->GET_PATH_TO_DOWNLOADED_UPDATE_FILE = 'https://github.com/lime-sklad/update_limestore/raw/master/testupdate.zip';
    }
    ...
    **/
    ...
    {
        return file_get_contents($this->GET_PATH_TO_LOCAL_VERSION_FILE);
    }
